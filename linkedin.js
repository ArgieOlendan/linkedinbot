...
const linkedin = {
    ...
    getEmployees: async (employeeType) => {
        ...
        await linkedin.page.evaluate(() => {
            let names = Array.from(document.querySelectorAll('div[class="org-people-profile-card__profile-title t-black lt-line-clamp lt-line-clamp--single-line ember-view"]')).map((el) => el.innerText);
            let possitions = Array.from(document.querySelectorAll('div[class="lt-line-clamp lt-line-clamp--multi-line ember-view"]')).map((el) => el.innerText);
            let linkedInProfiles = Array.from(document.querySelectorAll('ul.org-people-profiles-module__profile-list div[class="artdeco-entity-lockup__title ember-view"] a')).map((a) => a.href);
            ...
            names.forEach((name, index) => {
                employees.push({
                    name: name,
                    possition: possitions[index],
                    linkedInProfile: linkedInProfiles[index]
                })
            });
